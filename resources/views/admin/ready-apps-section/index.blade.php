...
@section('title', __('messages.ready_apps_section_management') ?? 'قسم التطبيقات الجاهزة')
@section('page-title', __('messages.ready_apps_section_management') ?? 'قسم التطبيقات الجاهزة')

@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>{{ __('messages.ready_apps_section_management') ?? 'قسم التطبيقات الجاهزة' }}</h2>
            <p>{{ __('messages.manage_ready_apps_section_desc') ?? 'إدارة قسم التطبيقات الجاهزة في الصفحة الرئيسية' }}</p>
        </div>
        ...
        <div class="card" style="margin-bottom: 24px;">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <button type="button" class="btn btn-sm btn-primary"
                    onclick="openEditSectionHeader('{{ route('admin.customer-facing.ready-apps-section.edit', $section) }}', 'sectionModal', '{{ __('messages.edit_section_header') ?? 'تعديل عنوان ووصف القسم' }}')">
    ...
    <!-- Ready Apps Categories List -->
    @if(isset($sectionCategories) && $sectionCategories->count() > 0)
            <div class="card">
                <div class="card-header">
                    <h3>{{ __('messages.ready_apps_list') ?? 'قائمة التطبيقات الجاهزة' }}</h3>
                </div>
                <div class="card-body">
                            ...
                            <div class="service-card" style="background: var(--card-bg, var(--sidebar-bg)); border-radius: 12px; overflow: hidden; box-shadow: var(--card-shadow, 0 2px 8px rgba(0,0,0,0.1)); border: 1px solid var(--border-color); transition: transform 0.2s, box-shadow 0.2s; position: relative;" 
                                ...
                                <div class="service-card-actions" style="position: absolute; top: 10px; left: 10px; z-index: 10; display: flex; gap: 8px;">
                                    <button type="button" 
                                            ...
                                            onclick="event.stopPropagation(); openEditCategoryModal({{ $category->id }}, 'categoryModal', '{{ __('messages.edit_ready_app') ?? 'تعديل التطبيق' }}')"
        ...
        <div class="card">
            <div class="card-body">
                <p class="text-center text-muted" style="padding: 60px;">
                    {{ __('messages.no_categories_available') ?? 'لا توجد تطبيقات جاهزة متاحة' }}
                </p>
            </div>
        </div>
    @endif
    ...
    <div class="modal-overlay" id="categoryModal" style="display: none;">
        <div class="modal-container">
            <div class="modal-header">
                <h3 class="modal-title" id="categoryModalTitle">{{ __('messages.edit_ready_app') ?? 'تعديل التطبيق' }}</h3>
    ...
    <script>
        ...
        function openEditCategoryModal(categoryId, modalId, title) {
            ...
            const url = '{{ route("admin.customer-facing.ready-apps-section.category.edit", ":id") }}'.replace(':id', categoryId);
