<form id="createAiVideoForm" method="POST" action="{{ route('admin.ai-videos.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group">
        ...
        <input type="url" name="video_url" id="video_url" class="form-control" value="{{ old('video_url') }}" required placeholder="https://youtube.com/watch?v=...">
        @error('video_url')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <input type="number" name="duration" id="duration" class="form-control" value="{{ old('duration') }}" min="0" placeholder="{{ __('messages.duration_example') }}">
        @error('duration')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="modal-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal('createAiVideoModal')">{{ __('messages.cancel') }}</button>
        <button type="submit" class="btn btn-primary">{{ __('messages.save') }}</button>
    </div>
</form>

<script>
    document.getElementById('createAiVideoForm').addEventListener('submit', function(e) {
        e.preventDefault();
