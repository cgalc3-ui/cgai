                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <button type="button" class="btn btn-sm btn-warning" onclick="openEditCustomerModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ addslashes($user->email ?? '') }}', '{{ addslashes($user->phone ?? '') }}', '{{ $user->date_of_birth ? $user->date_of_birth->format('Y-m-d') : '' }}', '{{ $user->gender ?? '' }}')">
                                <i class="fas fa-edit"></i> تعديل
                            </button>
                            <form action="{{ route('admin.users.customers.delete', $user) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا العميل؟')">
                                @csrf
                                @method('DELETE')
...
<x-modal modalId="addCustomerModal" title="إضافة عميل جديد" formId="addCustomerForm">
    <form id="addCustomerForm" action="{{ route('admin.users.customers.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="add_customer_name" name="name" value="{{ old('name') }}" class="form-control" required>
            @error('name')
        ...
        <div class="form-group">
            ...
            <input type="text" id="add_customer_phone" name="phone" value="{{ old('phone') }}" class="form-control" required>
            @error('phone')
                <span class="error-message">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <div class="date-input-wrapper">
                    <input type="date" id="add_customer_date_of_birth" name="date_of_birth" value="{{ old('date_of_birth') }}" class="form-control" max="{{ date('Y-m-d') }}">
    ...
    <form id="editCustomerForm" method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <div class="date-input-wrapper">
                    <input type="date" id="edit_customer_date_of_birth" name="date_of_birth" class="form-control" max="{{ date('Y-m-d') }}">
                    <i class="fas fa-calendar-alt date-icon"></i>
                </div>
                @error('date_of_birth')
                    <span class="error-message">{{ $message }}</span>
