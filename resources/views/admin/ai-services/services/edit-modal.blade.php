<form action="{{ route('admin.ai-services.services.update', $service->id) }}" method="POST"
    enctype="multipart/form-data" class="modal-form" id="editAiServiceForm">
    @csrf
    @method('PUT')
    ...
    <div class="form-group">
        ...
        <input type="text" id="name_en" name="name_en" class="form-control"
            value="{{ old('name_en', $service->name_en) }}" style="direction: ltr; text-align: left;">
        @error('name_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_free" id="is_free_checkbox" value="1" {{ old('is_free', $service->is_free) ? 'checked' : '' }}>
    ...
    <div class="form-group">
        ...
        <textarea id="short_description_en" name="short_description_en" class="form-control" rows="3"
            style="direction: ltr; text-align: left;">{{ old('short_description_en', $service->short_description_en) }}</textarea>
    ...
    <div class="form-group">
        ...
        <textarea id="full_description_en" name="full_description_en" class="form-control" rows="6"
            style="direction: ltr; text-align: left;">{{ old('full_description_en', $service->full_description_en) }}</textarea>
    ...
    <div class="form-group">
        ...
        @if($service->images->where('type', 'main')->first())
            <div style="margin-bottom: 15px;">
                <div class="selected-image-card existing">
                    <div class="image-wrapper">
                        <img src="{{ asset($service->images->where('type', 'main')->first()->url) }}"
    ...
    <div class="form-group">
        ...
        <div id="gallery_preview"
            class="selected-images-container {{ $service->images->where('type', 'gallery')->count() > 0 ? 'has-content' : '' }}">
            @foreach($service->images->where('type', 'gallery') as $image)
    ...
    <div class="form-group">
        ...
        <div id="screenshots_preview"
            class="selected-images-container {{ $service->screenshots->count() > 0 ? 'has-content' : '' }}">
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_new" value="1" {{ old('is_new', $service->is_new) ? 'checked' : '' }}>
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-secondary" onclick="closeModal('editAiServiceModal'); return false;">
...
<script>
    (function () {
        console.log("AI Services Edit Modal Script Initialized");
        ...
        // Form Submit Handler
        const form = document.getElementById('editAiServiceForm');
        if (form) {
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                handleFormSubmit(form, 'editAiServiceModal');
            });
        }
    })();
</script>
