...
@section('title', __('messages.ai_service_order'))
@section('page-title', __('messages.ai_service_order'))
    ...
    <div class="order-details-container">
        ...
        <header class="order-header">
            <div class="header-main">
                <h1 class="order-title">{{ __('messages.ai_service_order') }}</h1>
                <p class="order-subtitle">{{ __('messages.created_at') }}: {{ $order->created_at->format('Y-m-d H:i') }}</p>
            </div>
            <div class="header-actions">
                <a href="{{ route('admin.ai-services.orders.index') }}" class="btn-back">
        ...
        <div class="content-grid">
                ...
                <div class="info-card">
                    ...
                    <div class="card-body">
                        <div class="app-info-box">
                            <h3>{{ $order->service->trans('name') }}</h3>
                            <div class="app-meta">
                                @if($order->service->category)
                                    <span>
                                        <i class="far fa-folder"></i> {{ $order->service->category->trans('name') }}
                                    </span>
                                @endif
                                @if($order->service->mainImageUrl)
                                    <div class="app-image-preview" style="margin-top: 15px;">
                                        <img src="{{ asset($order->service->mainImageUrl) }}" alt="{{ $order->service->trans('name') }}" style="max-width: 200px; border-radius: 8px; border: 1px solid #e5e7eb;">
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            <div class="sidebar-column">
                ...
                <div class="action-card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.ai-services.orders.update-status', $order) }}" method="POST" class="status-form">
                            @csrf
                            @method('PUT')
                ...
                <div class="action-card">
                    ...
                    <div class="card-body">
                        <div class="quick-actions">
                            <a href="mailto:{{ $order->user->email }}?subject={{ __('messages.ai_service_order') }} #{{ $order->id }}" class="action-btn">
                            ...
                            @endif
                            <a href="{{ route('admin.ai-services.services.show', $order->service) ?? '#' }}" class="action-btn" target="_blank">
