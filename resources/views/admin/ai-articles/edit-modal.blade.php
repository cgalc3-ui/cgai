<form id="editAiArticleForm" method="POST" action="{{ route('admin.ai-articles.update', $aiArticle) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    
    <div class="form-group">
        <label for="title">{{ __('messages.title') }} <span class="required">*</span></label>
        <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $aiArticle->title) }}" required>
        @error('title')
    ...
    <div class="form-group">
        ...
        <input type="text" name="title_en" id="title_en" class="form-control" value="{{ old('title_en', $aiArticle->title_en) }}">
        @error('title_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="excerpt" id="excerpt" class="form-control" rows="3" maxlength="500">{{ old('excerpt', $aiArticle->excerpt) }}</textarea>
        @error('excerpt')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="excerpt_en" id="excerpt_en" class="form-control" rows="3" maxlength="500">{{ old('excerpt_en', $aiArticle->excerpt_en) }}</textarea>
        @error('excerpt_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="content" id="content" class="form-control" rows="10" required>{{ old('content', $aiArticle->content) }}</textarea>
        @error('content')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="content_en" id="content_en" class="form-control" rows="10">{{ old('content_en', $aiArticle->content_en) }}</textarea>
        @error('content_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        @if($aiArticle->image)
            <div style="margin-bottom: 10px;">
                <img src="{{ asset($aiArticle->image) }}" alt="Current image" style="max-width: 200px; border-radius: 8px;">
            </div>
        @endif
    ...
    <div class="form-group">
        ...
        <select name="category_id" id="category_id" class="form-control">
            ...
            @foreach($categories as $category)
                <option value="{{ $category->id }}" {{ old('category_id', $aiArticle->category_id) == $category->id ? 'selected' : '' }}>
    ...
    <div class="form-group">
        ...
        <input type="datetime-local" name="published_at" id="published_at" class="form-control" value="{{ old('published_at', $aiArticle->published_at ? $aiArticle->published_at->format('Y-m-d\TH:i') : '') }}">
        @error('published_at')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $aiArticle->is_featured) ? 'checked' : '' }}>
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $aiArticle->is_active) ? 'checked' : '' }}>
            <span>{{ __('messages.is_active') }}</span>
        </label>
    </div>

    <div class="modal-footer">
        <button type="button" class="btn btn-secondary" onclick="closeModal('editAiArticleModal')">{{ __('messages.cancel') }}</button>
        <button type="submit" class="btn btn-primary">{{ __('messages.save') }}</button>
    </div>
</form>

<script>
    document.getElementById('editAiArticleForm').addEventListener('submit', function(e) {
        e.preventDefault();
