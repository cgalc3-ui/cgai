<form action="{{ isset($navigationItem) ? route('admin.customer-facing.navigation.buttons.update', $navigationItem) : route('admin.customer-facing.navigation.buttons.store') }}" method="POST" id="buttonForm" class="modal-form">
    @csrf
    ...
    <div class="form-group">
        <label for="link">{{ __('messages.link') }} <span class="required">*</span></label>
        <input type="url" id="link" name="link" value="{{ old('link', $navigationItem->link ?? '') }}" class="form-control" required>
        @error('link')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-secondary" onclick="if(window.closeModal) window.closeModal('buttonModal'); return false;">
