<form action="{{ isset($hero) ? route('admin.customer-facing.hero.update', $hero) : route('admin.customer-facing.hero.store') }}" method="POST" id="heroForm" class="modal-form" enctype="multipart/form-data">
    @csrf
    @if(isset($hero))
        @method('PUT')
    @endif
    ...
    <div class="form-group">
        ...
        <input type="text" id="heading" name="heading" value="{{ old('heading', $hero->heading ?? '') }}" class="form-control" required>
        @error('heading')
    ...
    <div class="form-group">
        ...
        <input type="text" id="heading_en" name="heading_en" value="{{ old('heading_en', $hero->heading_en ?? '') }}" class="form-control" style="direction: ltr; text-align: left;">
        @error('heading_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label for="subheading">{{ __('messages.subheading') }} (AR)</label>
        <input type="text" id="subheading" name="subheading" value="{{ old('subheading', $hero->subheading ?? '') }}" class="form-control">
        @error('subheading')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label for="subheading_en">{{ __('messages.subheading') }} (EN)</label>
        <input type="text" id="subheading_en" name="subheading_en" value="{{ old('subheading_en', $hero->subheading_en ?? '') }}" class="form-control" style="direction: ltr; text-align: left;">
        @error('subheading_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description" name="description" class="form-control" rows="4">{{ old('description', $hero->description ?? '') }}</textarea>
        @error('description')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description_en" name="description_en" class="form-control" rows="4" style="direction: ltr; text-align: left;">{{ old('description_en', $hero->description_en ?? '') }}</textarea>
    ...
    <div class="form-group">
        ...
        @if(isset($hero) && $hero->background_image)
            <small class="form-help">
                <img src="{{ asset('storage/' . $hero->background_image) }}" alt="Current background" style="max-width: 200px; margin-top: 10px; border-radius: 8px;">
            </small>
        @endif
    ...
    <div class="form-group">
        ...
        <div id="buttons-container">
            @php
                $buttons = old('buttons', isset($hero) && $hero->buttons ? $hero->buttons : [['title' => '', 'title_en' => '', 'link' => '', 'target' => '_self', 'style' => 'primary']]);
            @endphp
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', isset($hero) ? $hero->is_active : true) ? 'checked' : '' }}>
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-secondary" onclick="if(window.closeModal) window.closeModal('heroModal'); return false;">
