...
<div class="page-header">
    <div class="page-header-left">
        ...
        <p>إنشاء باقة اشتراك جديدة في النظام</p>
    </div>
    ...
    <form action="{{ route('admin.subscriptions.store') }}" method="POST" id="subscriptionForm">
        ...
        <div class="form-group">
            ...
            <select id="duration_type" name="duration_type" class="form-control" required>
                <option value="monthly" {{ old('duration_type') == 'monthly' ? 'selected' : '' }}>شهري</option>
                <option value="3months" {{ old('duration_type') == '3months' ? 'selected' : '' }}>3 أشهر</option>
                <option value="6months" {{ old('duration_type') == '6months' ? 'selected' : '' }}>6 أشهر</option>
                <option value="yearly" {{ old('duration_type') == 'yearly' ? 'selected' : '' }}>سنوي</option>
            </select>
            @error('duration_type')
                <span class="error-message">{{ $message }}</span>
            @enderror
        ...
        <div class="form-group">
            ...
            <div id="features-container">
                <div class="feature-item" style="display: flex; gap: 10px; margin-bottom: 10px;">
                    <input type="text" name="features[0][name]" class="form-control" placeholder="اسم الميزة" required>
