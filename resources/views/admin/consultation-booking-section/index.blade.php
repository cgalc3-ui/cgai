...
@section('title', __('messages.consultation_booking_section_management') ?? 'قسم حجز الاستشارة')
@section('page-title', __('messages.consultation_booking_section_management') ?? 'قسم حجز الاستشارة')

@section('content')
    <div class="page-header">
        <div class="page-header-left">
            <h2>{{ __('messages.consultation_booking_section_management') ?? 'قسم حجز الاستشارة' }}</h2>
            <p>{{ __('messages.manage_consultation_booking_section_desc') ?? 'إدارة قسم حجز الاستشارة في الصفحة الرئيسية' }}</p>
        </div>
        <div class="page-header-right">
            @if($section)
                <button type="button" class="btn btn-primary"
                    onclick="openEditModal('{{ route('admin.customer-facing.consultation-booking-section.edit', $section) }}', 'sectionModal', '{{ __('messages.edit_consultation_booking_section') ?? 'تعديل قسم حجز الاستشارة' }}')">
                    <i class="fas fa-edit"></i> {{ __('messages.edit_consultation_booking_section') ?? 'تعديل' }}
                </button>
            @else
                <button type="button" class="btn btn-primary"
    ...
    @if($section)
        <div class="card">
            <div class="card-header">
                <h3>{{ __('messages.consultation_booking_section_preview') ?? 'معاينة قسم حجز الاستشارة' }}</h3>
            </div>
            <div class="card-body">
        ...
        <div class="card">
            <div class="card-body">
                <p class="text-center text-muted" style="padding: 60px;">
                    {{ __('messages.no_consultation_booking_section_set') ?? 'لم يتم تعيين قسم حجز الاستشارة بعد' }}
                </p>
            </div>
        </div>
    @endif
