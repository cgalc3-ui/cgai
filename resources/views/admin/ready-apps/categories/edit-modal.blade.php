<form action="{{ route('admin.ready-apps.categories.update', $category) }}" method="POST" id="readyAppCategoryEditForm" class="modal-form" enctype="multipart/form-data">
    @csrf
    @method('PUT')

    <div class="form-group">
        <label for="name">{{ __('messages.name') }} (AR) <span class="required">*</span></label>
        <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" class="form-control" placeholder="أدخل اسم الفئة بالعربية" required>
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description" name="description" class="form-control" rows="4" placeholder="أدخل وصف الفئة بالعربية">{{ old('description', $category->description) }}</textarea>
    </div>
    ...
    <div class="form-group">
        ...
        <textarea id="description_en" name="description_en" class="form-control" rows="4"
            style="direction: ltr; text-align: left;" placeholder="Enter category description in English">{{ old('description_en', $category->description_en) }}</textarea>
    </div>
        ...
        @if($category->image)
            <div style="margin-bottom: 10px;">
                <img src="{{ strpos($category->image, '/storage/') === 0 ? $category->image : asset('storage/' . $category->image) }}" 
                     alt="Category Image" 
                     style="width: 100px; height: 100px; object-fit: cover; border-radius: 8px; border: 1px solid rgba(255, 255, 255, 0.1);">
            </div>
        @endif
        <div class="custom-file-upload">
            <input type="file" id="image" name="image" class="form-control" accept="image/*">
            <div class="file-dummy">
                <span class="default">{{ $category->image ? 'تغيير الصورة' : 'اختر صورة' }}</span>
            </div>
        </div>
    </div>
    ...
    (function() {
        ...
        if (imageInput) {
            imageInput.addEventListener('change', function(e) {
                const fileName = e.target.files[0]?.name || (this.closest('form').querySelector('img') ? 'تغيير الصورة' : 'اختر صورة');
        ...
        const form = document.getElementById('readyAppCategoryEditForm');
        if (form) {
