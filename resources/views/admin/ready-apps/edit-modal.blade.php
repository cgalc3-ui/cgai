<form action="{{ route('admin.ready-apps.apps.update', $app->id) }}" method="POST" enctype="multipart/form-data"
    class="modal-form" id="editReadyAppForm">
    @csrf
    @method('PUT')

    <div class="form-group">
        <label for="name">{{ __('messages.name') }} (AR) <span class="required">*</span></label>
        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $app->name) }}" required>
        @error('name')
    ...
    <div class="form-group">
        <label for="name_en">{{ __('messages.name') }} (EN) <span class="required">*</span></label>
        <input type="text" id="name_en" name="name_en" class="form-control" value="{{ old('name_en', $app->name_en) }}"
    ...
    <div class="form-row">
        <div class="form-group">
            ...
            <select id="category_id" name="category_id" class="form-control" required>
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ old('category_id', $app->category_id) == $category->id ? 'selected' : '' }}>
                        {{ $category->trans('name') }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input type="number" id="price" name="price" class="form-control" value="{{ old('price', $app->price) }}"
                step="0.01" required>
            @error('price')
                <span class="error-message">{{ $message }}</span>
            @enderror
    ...
    <div class="form-group">
        ...
        <input type="number" id="original_price" name="original_price" class="form-control"
            value="{{ old('original_price', $app->original_price) }}" step="0.01">
    ...
    <div class="form-group">
        <label for="video_url">{{ __('messages.video_url') }} ({{ __('messages.optional') }})</label>
        <input type="url" id="video_url" name="video_url" class="form-control"
            value="{{ old('video_url', $app->video_url) }}" placeholder="https://www.youtube.com/watch?v=...">
        @error('video_url')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label for="short_description">{{ __('messages.short_description') }} (AR) <span
                class="required">*</span></label>
        <textarea id="short_description" name="short_description" class="form-control"
            rows="3">{{ old('short_description', $app->short_description) }}</textarea>
    ...
    <div class="form-group">
        <label for="short_description_en">{{ __('messages.short_description') }} (EN) <span
                class="required">*</span></label>
        <textarea id="short_description_en" name="short_description_en" class="form-control" rows="3"
            style="direction: ltr; text-align: left;">{{ old('short_description_en', $app->short_description_en) }}</textarea>
    ...
    <div class="form-group">
        ...
        <textarea id="full_description" name="full_description" class="form-control"
            rows="6">{{ old('full_description', $app->full_description) }}</textarea>
    ...
    <div class="form-group">
        ...
        <textarea id="full_description_en" name="full_description_en" class="form-control" rows="6"
            style="direction: ltr; text-align: left;">{{ old('full_description_en', $app->full_description_en) }}</textarea>
    ...
    <div class="form-group">
        <label for="main_image">{{ __('messages.main_image') }} ({{ __('messages.optional') }})</label>
        @if($app->images->where('type', 'main')->first())
            <div style="margin-bottom: 15px;">
                <div class="selected-image-card existing">
                    <div class="image-wrapper">
                        <img src="{{ asset($app->images->where('type', 'main')->first()->url) }}" alt="Current main image">
                    </div>
                </div>
                <p style="font-size: 12px; color: #666; margin-top: 8px;">{{ __('messages.current_image') }}</p>
            </div>
        @endif
    ...
    <div class="form-group">
        ...
        <div id="gallery_preview"
            class="selected-images-container {{ $app->images->where('type', 'gallery')->count() > 0 ? 'has-content' : '' }}">
            @foreach($app->images->where('type', 'gallery') as $image)
                <div class="selected-image-card existing" data-id="{{ $image->id }}">
                    <div class="image-wrapper">
                        <img src="{{ asset($image->url) }}" alt="Gallery image">
                    </div>
                    <button type="button" class="remove-image-btn"
                        onclick="removeExistingImageEx(this, 'gallery', {{ $image->id }})">×</button>
                </div>
            @endforeach
        </div>
    ...
    <div class="form-group">
        <label for="screenshots">{{ __('messages.ready_app_screenshots') }} ({{ __('messages.optional') }})</label>
        ...
        <div id="screenshots_preview"
            class="selected-images-container {{ $app->screenshots->count() > 0 ? 'has-content' : '' }}">
            @foreach($app->screenshots as $screenshot)
                <div class="selected-image-card existing" data-id="{{ $screenshot->id }}">
                    <div class="image-wrapper">
                        <img src="{{ asset($screenshot->url) }}" alt="Screenshot">
                    </div>
                    <button type="button" class="remove-image-btn"
                        onclick="removeExistingImageEx(this, 'screenshots', {{ $screenshot->id }})">×</button>
                </div>
            @endforeach
        </div>
        <div id="deleted_screenshots_container"></div>
    </div>

    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $app->is_featured) ? 'checked' : '' }}>
    ...
    <div class="form-group">
        <label class="checkbox-label">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $app->is_active) ? 'checked' : '' }}>
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-secondary" onclick="closeModal('editReadyAppModal'); return false;">
...
<script>
    (function () {
        console.log("Ready Apps Edit Modal Script Initialized");

        const galleryFiles = [];
        const screenshotsFiles = [];

        // Globalized helper for inline onclick
        window.removeExistingImageEx = function (btn, type, id) {
            const card = btn.closest('.selected-image-card');
            const inputName = type === 'gallery' ? 'deleted_gallery_images' : 'deleted_screenshots';
            const container = document.getElementById(`deleted_${type}_container`);
            ...
            // Trigger re-render to handle container visibility
            renderPreview(type);
        }
        ...
        function renderPreview(type) {
            ...
            // 1. Keep the existing cards (those with .existing class)
            const existingCards = Array.from(previewContainer.querySelectorAll('.selected-image-card.existing'));

            // 2. Clear the container
            previewContainer.innerHTML = '';

            // 3. Re-append existing cards
            existingCards.forEach(card => previewContainer.appendChild(card));

            // 4. Render and append NEW cards
            ...
            // 5. Update visibility
            if (previewContainer.querySelectorAll('.selected-image-card').length > 0) {
        ...
        if (mainInput) {
            mainInput.addEventListener('change', function (e) {
                ...
                if (file) {
                    ...
                    // Don't remove for edit if there was an existing one, 
                    // but here we only handle the new one's preview
                }
            });
        }
