...
@section('content')
    ...
    <div class="table-container">
                        ...
                        <td>
                            <div style="display: flex; gap: 8px;">
                                ...
                                <form action="{{ route('admin.customers.delete', $customer) }}" method="POST" class="d-inline"
                                    onsubmit="event.preventDefault(); Confirm.delete({{ json_encode(__('messages.delete_customer_confirm')) }}, {{ json_encode(__('messages.confirm_delete_title')) }}).then(confirmed => { if(confirmed) this.submit(); }); return false;">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="pagination-wrapper">
            {{ $customers->links('vendor.pagination.custom', ['itemName' => 'customers']) }}
        </div>
    </div>
@endsection