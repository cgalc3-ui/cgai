<form action="{{ route('admin.help-guides.store') }}" method="POST" id="helpGuideCreateForm" class="modal-form">
    @csrf
    ...
    <div class="form-group">
        ...
        <select id="role" name="role" class="form-control" required>
            ...
            <option value="customer" {{ old('role') == 'customer' ? 'selected' : '' }}>{{ __('messages.customer_role') }}</option>
        </select>
        @error('role')
    ...
    <div class="form-group">
        ...
        <textarea id="content_en" name="content_en" rows="6" class="form-control"
            style="direction: ltr; text-align: left;">{{ old('content_en') }}</textarea>
        @error('content_en')
            <span class="error-message">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" id="icon" name="icon" value="{{ old('icon', 'fas fa-info-circle') }}" 
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-secondary" onclick="closeModal('createHelpGuideModal'); return false;">
...
<script>
    (function() {
        const form = document.getElementById('helpGuideCreateForm');
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    if (data.success) {
                        closeModal('createHelpGuideModal');
                        if (data.redirect) {
